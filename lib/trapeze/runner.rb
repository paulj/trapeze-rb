...
module Trapeze  
  # AMQP Webfront overrides of the AMQP Webfront runner
  class Runner < Thin::Runner
    def parser
      # Load in AMQP Webfront specific defaults
      # Default options values
      @options[:address] = AMQPConnector::DEFAULT_SERVER
      @options[:label] = AMQPConnector::DEFAULT_QNAME
      @options[:log] = 'log/trapeze-rb.log'
      @options[:pid] = 'tmp/pids/trapeze-rb.pid'
      @options[:backend] = Trapeze::ThinBackend.name
      @options[:key] = AMQPConnector::DEFAULT_BINDING
      ...
      @parser ||= OptionParser.new do |opts|
        opts.banner = "Usage: trapezerb [options] #{self.class.commands.join('|')}"

        opts.separator ""
        opts.separator "Server options:"

        opts.on("-a", "--address SERVER", "use SERVER for AMQP connectivity " +
        ...
        opts.on("-k", "--key KEY", "use KEY (default: #{@options[:key]})")              { |binding| @options[:key] = binding }
